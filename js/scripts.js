...
$.getJSON(facsubset_url, function(facsubset_data) {
  ...
  for (var i=0; i<facsubset_data.features.length; i++) {
    var myRow = '<tr>'
      + '<td>' + facsubset_data.features[i].properties.facname + '</td>'
      + '<td>' + facsubset_data.features[i].properties.facsubgrp + '</td>'
      + '<td>' + facsubset_data.features[i].properties.address + '</td>'
      + '<td><a href=\'https://capitalplanning.nyc/facility/' + facsubset_data.features[i].properties.uid + '\' target=\'_blank\'>Link to more details</a></td>'
    + '</tr>';
